...
class CNN: 
	...
	def __init__(self, data, target, keep_prob, add_tensorboard_summary = True, scope_name='CNN', one_hot_labels = True):
		...
		self.filter_dims 		= config_dict['filter_dims'] 		# height and width of the conv kernels 	for each layer
		self.hidden_channels 	= config_dict['hidden_channels']	# number of feature maps 				for each layer

		pooling_type = config_dict['pooling_type']

		if pooling_type == 'strided_conv':
			# use strides as downsampling
			self.strides = [[1,2,2,1] for filter in config_dict['filter_dims'] ]
		else:
			# default value
			self.strides = [[1,1,1,1] for filter in config_dict['filter_dims'] ]

		# layer sizes for the dense layers (decision making)
		self.dense_depths = []
		self.dense_depths.extend(config_dict['dense_depths'])
		...
		self.activation_function	= config_dict['activation_function']
